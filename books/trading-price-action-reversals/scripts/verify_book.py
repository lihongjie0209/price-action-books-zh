...
def main() -> int:
    ...
    if "trading-price-action-reversals" not in ids:
        errors.append("catalog missing trading-price-action-reversals")
    else:
        b = next(x for x in cat["books"] if x["id"] == "trading-price-action-reversals")
        if not b.get("toc"):
            errors.append("catalog missing hierarchical toc")

    samples = [
        "03-introduction.md",
        "07-part-i-trend-reversals-a-trend-becoming-an-opposite-trend.md",
        "34-ch24-the-best-trades-putting-it-all-together.md",
    ]
    for s in samples:
